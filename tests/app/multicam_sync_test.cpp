...
TEST(MulticamTimecodeSyncTest, ProposesOffsetsFromEarliestSourceClock) {
  ...
  EXPECT_EQ(proposal.angles[1].sync_offset, edit::Time(1'500'000, 1'000'000));
}

TEST(MulticamTimecodeSyncTest, IncludesSourceInPointInClock) {
  edit::Project project;
  edit::Asset asset_a;
  asset_a.metadata["timecode_start_us"] = "1000000";
  asset_a.duration = edit::Time(100, 1);
  ...
  clip_a.asset_id = asset_a.id;
  clip_a.kind = edit::ClipKind::Video;
  clip_a.timeline_range = edit::TimeRange(edit::Time{}, edit::Time(50, 1));
  clip_a.source_range = edit::TimeRange(edit::Time(2, 1), edit::Time(50, 1));
  edit::Clip clip_b = clip_a;
  clip_b.id = edit::EntityId::generate();
  clip_b.asset_id = asset_b.id;
  clip_b.source_range = edit::TimeRange(edit::Time{}, edit::Time(50, 1));
  track_a.clips.push_back(clip_a);
  track_b.clips.push_back(clip_b);
  sequence.tracks = {track_a, track_b};
  project.sequences.push_back(sequence);
  ...
  const auto proposal = proposeMulticamTimecodeSync(project, sequence, group, edit::Time{});
  ASSERT_FALSE(proposal.requires_manual_choice);
  ASSERT_EQ(proposal.angles.size(), 2U);
  // A visible clock = 1s + 2s in-point = 3s; B = 2.5s. Earliest is B.
  EXPECT_EQ(proposal.angles[0].sync_offset, edit::Time(500'000, 1'000'000));
  EXPECT_EQ(proposal.angles[1].sync_offset, edit::Time(0, 1'000'000));
}
