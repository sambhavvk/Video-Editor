...
[[noreturn]] void fail(CodecErrorCode code, std::string field_path,
                       std::string message) {
  throw CodecException(
      CodecError{code, std::move(message), std::move(field_path)});
}
...
class IdRegistry final {
 public:
  void add(edit::EntityId id, std::string_view path) {
    require(!id.isNil(), CodecErrorCode::InvalidField, path,
...
void encodeId(edit::EntityId value, wire::EntityId* output,
              std::string_view path, IdRegistry* registry = nullptr,
...
void encodeVec2(const edit::Vec2& value, wire::Vec2* output,
...
void encodeColor(const edit::ColorRgba& value, wire::ColorRgba* output,
                 std::string_view path) {
...
void encodeMetadata(const std::map<std::string, std::string, std::less<>>& value,
  ...
  for (const auto& [key, item] : value) {
    require(!key.empty(), CodecErrorCode::InvalidField, path,
...
[[nodiscard]] wire::KeyframeInterpolation encodeInterpolation(
    edit::KeyframeInterpolation value) {
  switch (value) {
  ...
  fail(CodecErrorCode::InvalidField, "keyframe.interpolation",
...
void encodeKeyframe(const edit::Keyframe& value, wire::Keyframe* output,
                    std::string_view path, IdRegistry& ids) {
  encodeId(value.id, output->mutable_id(), childPath(path, "id"), &ids);
  require(!value.time.isNegative(), CodecErrorCode::InvalidField,
          childPath(path, "time"), "keyframe time cannot be negative");
  encodeTime(value.time, output->mutable_time());
  encodeEffectValue(value.value, output->mutable_value(),
...
void encodeParameter(const edit::EffectParameter& value,
                     wire::EffectParameter* output, std::string_view path,
                     IdRegistry& ids) {
  ...
  for (std::size_t index = 0; index < value.keyframes.size(); ++index) {
    ...
    if (previous_time) {
      require(value.keyframes[index].time > *previous_time,
              CodecErrorCode::InvalidField, childPath(keyframe_path, "time"),
              "keyframes must be strictly ordered by time");
    }
    encodeKeyframe(value.keyframes[index], output->add_keyframes(), keyframe_path,
                   ids);
    previous_time = value.keyframes[index].time;
  }
}

void encodeEffect(const edit::Effect& value, wire::Effect* output,
                  std::string_view path, IdRegistry& ids) {
  encodeId(value.id, output->mutable_id(), childPath(path, "id"), &ids);
  require(!value.type.empty(), CodecErrorCode::InvalidField,
          childPath(path, "type"), "effect type cannot be empty");
  require(value.version > 0, CodecErrorCode::InvalidField,
          childPath(path, "version"), "effect version must be non-zero");
  ...
  output->set_opaque_payload(value.opaque_payload.data(),
                             value.opaque_payload.size());
}

void encodeTransform(const edit::Transform& value, wire::Transform* output,
                     std::string_view path) {
  encodeVec2(value.position, output->mutable_position(),
             childPath(path, "position"));
  ...
  require(value.opacity >= 0.0 && value.opacity <= 1.0,
          CodecErrorCode::InvalidField, childPath(path, "opacity"),
          "opacity must be between zero and one");
  require(value.crop_left >= 0.0 && value.crop_left <= 1.0 &&
              value.crop_top >= 0.0 && value.crop_top <= 1.0 &&
              value.crop_right >= 0.0 && value.crop_right <= 1.0 &&
              value.crop_bottom >= 0.0 && value.crop_bottom <= 1.0,
          CodecErrorCode::InvalidField, path,
          "crop values must be between zero and one");
...
void encodeAsset(const edit::Asset& value, wire::Asset* output,
                 std::string_view path, IdRegistry& ids) {
  encodeId(value.id, output->mutable_id(), childPath(path, "id"), &ids);
  require(!value.duration.isNegative(), CodecErrorCode::InvalidField,
          childPath(path, "duration"), "asset duration cannot be negative");
  if (value.has_video) {
    require(value.width > 0 && value.height > 0, CodecErrorCode::InvalidField,
            path, "video assets require non-zero dimensions");
  }
  if (value.has_audio) {
    require(value.audio_sample_rate > 0 && value.audio_channels > 0,
            CodecErrorCode::InvalidField, path,
            "audio assets require a sample rate and channel count");
  }
  ...
  encodeMetadata(value.metadata, output->mutable_metadata(),
                 childPath(path, "metadata"));
}

void encodeClip(const edit::Clip& value, wire::Clip* output,
                std::string_view path, IdRegistry& ids) {
  encodeId(value.id, output->mutable_id(), childPath(path, "id"), &ids);
  encodeId(value.asset_id, output->mutable_asset_id(),
           childPath(path, "asset_id"), nullptr,
  ...
  if (value.linked_group) {
    encodeId(*value.linked_group, output->mutable_linked_group(),
             childPath(path, "linked_group"));
  }
  encodeTransform(value.transform, output->mutable_transform(),
                  childPath(path, "transform"));
  ...
  require(value.audio_pan >= -1.0 && value.audio_pan <= 1.0,
          CodecErrorCode::InvalidField, childPath(path, "audio_pan"),
          "audio pan must be between minus one and one");
  require(!value.fade_in.isNegative() && !value.fade_out.isNegative(),
  ...
  require(value.fade_in + value.fade_out <= value.timeline_range.duration,
          CodecErrorCode::InvalidField, path,
          "clip fades cannot exceed the clip duration");
  ...
  for (std::size_t index = 0; index < value.effects.size(); ++index) {
    encodeEffect(value.effects[index], output->add_effects(),
                 indexedPath(path, "effects", index), ids);
  }
}

void encodeTrack(const edit::Track& value, wire::Track* output,
                 std::string_view path, IdRegistry& ids) {
  encodeId(value.id, output->mutable_id(), childPath(path, "id"), &ids);
  ...
  for (std::size_t index = 0; index < value.clips.size(); ++index) {
    ...
    if (previous != nullptr) {
      require(previous->timeline_range.start <= value.clips[index].timeline_range.start,
              CodecErrorCode::InvalidProject, clip_path,
              "clips must be sorted by timeline start");
      require(!previous->timeline_range.overlaps(value.clips[index].timeline_range),
              CodecErrorCode::InvalidProject, clip_path,
              "clips on one track cannot overlap");
    }
...
void encodeMarker(const edit::Marker& value, wire::Marker* output,
                  std::string_view path, IdRegistry& ids) {
  encodeId(value.id, output->mutable_id(), childPath(path, "id"), &ids);
...
void encodeCaptionStyle(const edit::CaptionStyle& value,
                        wire::CaptionStyle* output, std::string_view path) {
  ...
  require(value.font_size > 0.0, CodecErrorCode::InvalidField,
          childPath(path, "font_size"), "caption font size must be positive");
  output->set_font_family(value.font_family);
  output->set_font_size(value.font_size);
  encodeColor(value.text_color, output->mutable_text_color(),
              childPath(path, "text_color"));
...
void encodeCaption(const edit::Caption& value, wire::Caption* output,
                   std::string_view path, IdRegistry& ids) {
  encodeId(value.id, output->mutable_id(), childPath(path, "id"), &ids);
  ...
  encodeCaptionStyle(value.style, output->mutable_style(),
                     childPath(path, "style"));
}

void encodeSequence(const edit::Sequence& value, wire::Sequence* output,
                    std::string_view path, IdRegistry& ids) {
  encodeId(value.id, output->mutable_id(), childPath(path, "id"), &ids);
  ...
  for (std::size_t index = 0; index < value.tracks.size(); ++index) {
    encodeTrack(value.tracks[index], output->add_tracks(),
                indexedPath(path, "tracks", index), ids);
  }
  for (std::size_t index = 0; index < value.markers.size(); ++index) {
    encodeMarker(value.markers[index], output->add_markers(),
                 indexedPath(path, "markers", index), ids);
  }
...
void encodeProject(const edit::Project& value, wire::Project* output) {
  ...
  for (std::size_t index = 0; index < value.assets.size(); ++index) {
    encodeAsset(value.assets[index], output->add_assets(),
                indexedPath("project", "assets", index), ids);
  }
...
[[nodiscard]] std::optional<std::string> findUnknownField(
    const google::protobuf::Message& message, std::string_view path) {
  ...
  for (const auto* field : fields) {
    ...
    if (field->is_repeated()) {
      ...
      for (int index = 0; index < size; ++index) {
        if (auto nested = findUnknownField(
      ...
      if (auto nested = findUnknownField(reflection->GetMessage(message, field),
                                         field_path)) {
        return nested;
      }
    }
...
[[nodiscard]] edit::EntityId decodeId(const wire::EntityId& value,
                                      std::string_view path,
...
[[nodiscard]] edit::Time decodeTime(const wire::Time& value,
                                    std::string_view path) {
  require(value.timescale() != 0, CodecErrorCode::InvalidField,
          childPath(path, "timescale"), "time timescale must be non-zero");
  return edit::Time(value.value(), value.timescale());
}

[[nodiscard]] edit::TimeRange decodeRange(const wire::TimeRange& value,
                                          std::string_view path) {
  ...
  require(!duration.isNegative(), CodecErrorCode::InvalidField,
          childPath(path, "duration"), "time range duration cannot be negative");
  return edit::TimeRange(start, duration);
}

[[nodiscard]] edit::Rate decodeRate(const wire::Rate& value,
                                    std::string_view path) {
  require(value.numerator() != 0 && value.denominator() != 0,
          CodecErrorCode::InvalidField, path,
          "rate numerator and denominator must be non-zero");
  return edit::Rate(value.numerator(), value.denominator());
}

[[nodiscard]] edit::Vec2 decodeVec2(const wire::Vec2& value,
                                    std::string_view path) {
  requireFinite(value.x(), childPath(path, "x"));
  requireFinite(value.y(), childPath(path, "y"));
...
[[nodiscard]] edit::ColorRgba decodeColor(const wire::ColorRgba& value,
                                          std::string_view path) {
  requireFinite(value.red(), childPath(path, "red"));
  requireFinite(value.green(), childPath(path, "green"));
  requireFinite(value.blue(), childPath(path, "blue"));
...
[[nodiscard]] std::map<std::string, std::string, std::less<>> decodeMetadata(
    const google::protobuf::RepeatedPtrField<wire::StringEntry>& entries,
    std::string_view path) {
  std::map<std::string, std::string, std::less<>> result;
  std::size_t index = 0;
  for (const auto& entry : entries) {
    const auto entry_path = indexedPath(path, "entries", index++);
    require(!entry.key().empty(), CodecErrorCode::InvalidField,
            childPath(entry_path, "key"), "metadata key cannot be empty");
...
[[nodiscard]] edit::KeyframeInterpolation decodeInterpolation(
    wire::KeyframeInterpolation value, std::string_view path) {
  switch (value) {
...
[[nodiscard]] edit::EffectValue decodeEffectValue(
    const wire::EffectValue& value, std::string_view path) {
  ...
  fail(CodecErrorCode::MissingField, std::string(path),
       "effect value oneof is not set");
}

[[nodiscard]] edit::Keyframe decodeKeyframe(const wire::Keyframe& value,
                                            std::string_view path,
                                            IdRegistry& ids) {
  ...
  requirePresent(value.has_incoming_control(),
                 childPath(path, "incoming_control"));
  requirePresent(value.has_outgoing_control(),
                 childPath(path, "outgoing_control"));
  ...
  require(!result.time.isNegative(), CodecErrorCode::InvalidField,
          childPath(path, "time"), "keyframe time cannot be negative");
...
[[nodiscard]] edit::EffectParameter decodeParameter(
    const wire::EffectParameter& value, std::string_view path, IdRegistry& ids) {
  require(!value.id().empty(), CodecErrorCode::InvalidField,
          childPath(path, "id"), "effect parameter id cannot be empty");
  requirePresent(value.has_value(), childPath(path, "value"));
...
[[nodiscard]] edit::Effect decodeEffect(const wire::Effect& value,
                                        std::string_view path,
                                        IdRegistry& ids) {
  requirePresent(value.has_id(), childPath(path, "id"));
  require(!value.type().empty(), CodecErrorCode::InvalidField,
...
[[nodiscard]] edit::Transform decodeTransform(const wire::Transform& value,
                                              std::string_view path) {
...
[[nodiscard]] edit::TrackKind decodeTrackKind(wire::TrackKind value,
                                              std::string_view path) {
  switch (value) {
  ...
  fail(CodecErrorCode::InvalidField, std::string(path),
       "track kind is unspecified or unknown");
}

[[nodiscard]] edit::ClipKind decodeClipKind(wire::ClipKind value,
                                            std::string_view path) {
  switch (value) {
  ...
  fail(CodecErrorCode::InvalidField, std::string(path),
       "clip kind is unspecified or unknown");
}

[[nodiscard]] edit::BlendMode decodeBlendMode(wire::BlendMode value,
                                              std::string_view path) {
  switch (value) {
  ...
  fail(CodecErrorCode::InvalidField, std::string(path),
       "blend mode is unspecified or unknown");
}

[[nodiscard]] edit::Asset decodeAsset(const wire::Asset& value,
                                      std::string_view path, IdRegistry& ids) {
  requirePresent(value.has_id(), childPath(path, "id"));
  requirePresent(value.has_duration(), childPath(path, "duration"));
  edit::Asset result;
  ...
  require(!result.duration.isNegative(), CodecErrorCode::InvalidField,
          childPath(path, "duration"), "asset duration cannot be negative");
  ...
  if (result.has_video) {
    require(result.width > 0 && result.height > 0, CodecErrorCode::InvalidField,
            path, "video assets require non-zero dimensions");
  }
  if (value.has_nominal_frame_rate()) {
    result.nominal_frame_rate = decodeRate(
        value.nominal_frame_rate(), childPath(path, "nominal_frame_rate"));
  }
  result.audio_sample_rate = value.audio_sample_rate();
  result.audio_channels = value.audio_channels();
  if (result.has_audio) {
    require(result.audio_sample_rate > 0 && result.audio_channels > 0,
            CodecErrorCode::InvalidField, path,
            "audio assets require a sample rate and channel count");
  }
...
[[nodiscard]] edit::Clip decodeClip(const wire::Clip& value,
                                    std::string_view path, IdRegistry& ids) {
  requirePresent(value.has_id(), childPath(path, "id"));
  ...
  result.asset_id = decodeId(value.asset_id(), childPath(path, "asset_id"),
                             nullptr, result.kind == edit::ClipKind::Title);
  result.name = value.name();
  result.timeline_range =
      decodeRange(value.timeline_range(), childPath(path, "timeline_range"));
  result.source_range =
      decodeRange(value.source_range(), childPath(path, "source_range"));
  result.playback_rate =
      decodeRate(value.playback_rate(), childPath(path, "playback_rate"));
  result.reversed = value.reversed();
  if (value.has_linked_group()) {
    result.linked_group =
        decodeId(value.linked_group(), childPath(path, "linked_group"));
  }
  result.transform =
      decodeTransform(value.transform(), childPath(path, "transform"));
  result.blend_mode =
      decodeBlendMode(value.blend_mode(), childPath(path, "blend_mode"));
  requireFinite(value.audio_gain_db(), childPath(path, "audio_gain_db"));
  requireFinite(value.audio_pan(), childPath(path, "audio_pan"));
  require(value.audio_pan() >= -1.0 && value.audio_pan() <= 1.0,
          CodecErrorCode::InvalidField, childPath(path, "audio_pan"),
  ...
  for (const auto& effect : value.effects()) {
    result.effects.push_back(
        decodeEffect(effect, indexedPath(path, "effects", index++), ids));
  }
  return result;
}

[[nodiscard]] edit::Track decodeTrack(const wire::Track& value,
                                      std::string_view path, IdRegistry& ids) {
  requirePresent(value.has_id(), childPath(path, "id"));
  edit::Track result;
  result.id = decodeId(value.id(), childPath(path, "id"), &ids);
  ...
  for (const auto& clip : value.clips()) {
    const auto clip_path = indexedPath(path, "clips", index++);
    auto decoded = decodeClip(clip, clip_path, ids);
    if (previous != nullptr) {
...
[[nodiscard]] edit::Marker decodeMarker(const wire::Marker& value,
                                        std::string_view path, IdRegistry& ids) {
  requirePresent(value.has_id(), childPath(path, "id"));
...
[[nodiscard]] edit::CaptionStyle decodeCaptionStyle(
    const wire::CaptionStyle& value, std::string_view path) {
  requirePresent(value.has_text_color(), childPath(path, "text_color"));
  requirePresent(value.has_background_color(),
                 childPath(path, "background_color"));
  require(!value.font_family().empty(), CodecErrorCode::InvalidField,
          childPath(path, "font_family"), "caption font family cannot be empty");
  requireFinite(value.font_size(), childPath(path, "font_size"));
  ...
  result.text_color =
      decodeColor(value.text_color(), childPath(path, "text_color"));
...
[[nodiscard]] edit::Caption decodeCaption(const wire::Caption& value,
                                          std::string_view path,
                                          IdRegistry& ids) {
  requirePresent(value.has_id(), childPath(path, "id"));
  requirePresent(value.has_range(), childPath(path, "range"));
...
[[nodiscard]] edit::Sequence decodeSequence(const wire::Sequence& value,
                                            std::string_view path,
                                            IdRegistry& ids) {
  requirePresent(value.has_id(), childPath(path, "id"));
  ...
  result.name = value.name();
  result.frame_rate =
      decodeRate(value.frame_rate(), childPath(path, "frame_rate"));
  result.width = value.width();
  result.height = value.height();
  result.audio_sample_rate = value.audio_sample_rate();
  std::size_t index = 0;
  for (const auto& track : value.tracks()) {
    result.tracks.push_back(
        decodeTrack(track, indexedPath(path, "tracks", index++), ids));
  }
  index = 0;
  for (const auto& marker : value.markers()) {
    result.markers.push_back(
        decodeMarker(marker, indexedPath(path, "markers", index++), ids));
  }
  index = 0;
  for (const auto& caption : value.captions()) {
    result.captions.push_back(
        decodeCaption(caption, indexedPath(path, "captions", index++), ids));
  }
  return result;
}
...
[[nodiscard]] edit::Project decodeProject(const wire::Project& value) {
  ...
  for (const auto& asset : value.assets()) {
    result.assets.push_back(
        decodeAsset(asset, indexedPath("project", "assets", index++), ids));
  }
  index = 0;
  for (const auto& sequence : value.sequences()) {
    result.sequences.push_back(decodeSequence(
        sequence, indexedPath("project", "sequences", index++), ids));
  }
...
CodecException::CodecException(CodecError error)
    : std::runtime_error(error.field_path.empty()
                             ? error.message
...
ProjectBytes serialize_project(const edit::Project& project) {
  try {
    ...
    require(size <= kMaximumSnapshotBytes, CodecErrorCode::SerializationFailed,
            "snapshot", "serialized project exceeds the snapshot size limit");
    ...
    google::protobuf::io::ArrayOutputStream array_output(result.data(),
...
edit::Result<edit::Project, CodecError> deserialize_project(
    std::span<const std::byte> bytes) {
  try {
    require(bytes.size() <= kMaximumSnapshotBytes,
    ...
    require(snapshot.schema_version() != 0, CodecErrorCode::MissingField,
            "snapshot.schema_version", "schema version is missing");
    require(snapshot.minimum_reader_version() != 0,
            CodecErrorCode::MissingField, "snapshot.minimum_reader_version",
            "minimum reader version is missing");
    if (snapshot.minimum_reader_version() > kCurrentSchemaVersion) {
      fail(CodecErrorCode::UnsupportedMinimumReaderVersion,
           "snapshot.minimum_reader_version",
           "snapshot requires a newer project reader");
    }
    if (snapshot.schema_version() != kCurrentSchemaVersion) {
    ...
    return edit::Result<edit::Project, CodecError>::success(
        decodeProject(snapshot.project()));
