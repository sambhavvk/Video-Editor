...
| Proxy generation | Partial | Recommended 4K long-GOP media is auto-queued one job at a time; manual Create editing proxy remains. Completed proxies and `.vepts` maps are adopted into `CacheStore` and rediscovered on reopen (legacy `{assetId}.proxy.*` is a read-only fallback). Generation is still in-process `QtConcurrent`; worker IPC and `.vepts`-driven VFR seek remain. |
...
| Cache management | Implemented | One `CacheStore` budget (default 100 GB, persisted in `QSettings`) covers thumbnails, waveforms, metadata, proxies, and PTS maps. `File > Manage Media Cache…` inspects LRU inventory, changes the budget, removes entries/assets, evicts to budget, and clears artifacts without touching `.veproj` or originals. `CacheErrorCode::Full` stops remaining cache jobs. Broader disk-full fault injection remains a release gate. |
...
transcription validation plus restartable proxy/export workers; and the packaging, accessibility,
codec/GPU corpus, endurance, security, and legal gates.
