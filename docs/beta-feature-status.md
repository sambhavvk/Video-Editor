...
| Track-based editing breadth | Partial | Unlimited model vectors plus desktop create/rename/reorder/remove, lock, visual visibility, targeting, markers, derived-gap interaction, title/transition authoring, speed controls, and track gain/pan are implemented. Double-clicking media loads the source monitor; **I**/**O** mark in/out, **L** ripple-inserts and **Comma** overwrites the marked range (or the full asset) onto targeted unlocked tracks with linked A/V. Large-track release performance evidence remains incomplete. |
| Media import and probing | Implemented | Async file/dialog/drop import, fingerprints, FFmpeg descriptors, searchable media bin, linked A/V insertion. Still images hold the last decoded frame for the clip duration (default 5 s). Numbered image sequences (`name_%04d.ext`) import as one `image2` asset. A synthetic 200+ lavfi corpus generator covers required categories; a physical decode lab is still a gate. |
...
| GPU preview and color management | Partial | A pinned, capability-gated libplacebo backend provides Vulkan on Linux and D3D11 on Windows, truthful unavailable stubs, device-loss state, CPU fallback, and per-clip GPU preview for active video/title clips with crop/position/scale/anchor, rotation with a moved pivot, opacity, the five premultiplied blend modes, supported clip color/crop/blur effects, and Cross Dissolve/Dip to Black transitions. Unknown enabled clip effects still fall back per frame to CPU without latching the GPU. On Linux the program viewer can present through a UI-owned Vulkan swapchain when a surface is available; the source monitor stays on the QImage path and download remains the CI/offscreen fallback. Backend/device failures preserve that frame's CPU result and latch CPU preview for the session. Device creation runs off the UI thread after CPU preview is already available. Windows still uses offscreen create/download. Native GPU effect/color shaders, YUV/native imports, hardware decode/encode, zero-copy, HDR tone mapping, and adaptive quality remain missing. The **first-beta hardware matrix is Linux Vulkan** (Intel/AMD/NVIDIA). The Windows D3D11 GPU matrix is deferred with the Windows beta. |
| Transform, blend, effects, and animation | Partial | Transform/crop/opacity and five blend modes are revisioned and CPU-rendered. The Effects panel creates typed color, curves, LUT, crop, and blur effects (clip audio DSP is mixer-only); the Inspector exposes signed scale, current values plus clip-local Hold/Linear/Bezier keyframe lists, exact fields, deletion, and a curve editor. CPU preview/export share the evaluator and expensive blur is bypassed only when its preview profile allows it; `video.lut` and `video.curves` render on CPU and trigger per-frame GPU fallback. A white-balance eyedropper writes temperature/tint on `video.color`. Native GPU effect shaders remain. |
| Titles, transitions, speed/reverse | Implemented | Title creation and Inspector text/font/size/alignment/emphasis controls, transition handles/presets/duration/removal, and 0.01x–100x/reverse controls are revisioned, undoable, schema-v3 persistent, and CPU preview/export rendered. The GPU path matches CPU for titles and Cross Dissolve/Dip to Black transitions; native GPU text/transition shaders and higher-quality pitch-preserving retime remain later optimizations, not separate authoring models. |
| Rec.709 color controls and LUTs | Partial | Known CPU effects expose exposure, temperature/tint (including white-balance eyedropper), contrast, saturation, RGB/luma curves, and Adobe `.cube` LUT import on CPU. GPU preview falls back per frame for LUT/curves. Metadata-preserving transforms, HDR-to-SDR completeness, and GPU parity remain missing. |
| Timeline audio render and clip properties | Partial | Originals-only decode/resample renders exact half-open 48 kHz stereo blocks with gaps/overlaps, clip and track gain/pan, fades, mute/solo, rate/reverse, ordered track DSP, and cancellation; export and realtime pre-render consume it. Forward 1× and J/K/L shuttle rates of 0.5×, 2×, 4×, and 8× (including reverse) use a bounded ring, selected miniaudio device, latency-compensated audio-master clock scaled by transport rate, serialized versioned controls, explicit silent fallback, underrun diagnostics, miniaudio event-driven hot-plug notifications with a 7.5 s backup poll while a device is open, and one-second polling when idle or the backend is unavailable. Per-device output latency calibration is stored in QSettings and applied on playback start; residual uncertainty remains. Pitch-preserving time-stretch and physical release-matrix endurance remain missing. |
| Mixer, DSP, meters, normalization | Partial | The Audio workspace exposes revisioned track gain/pan/mute/solo and current-value controls for parametric EQ, compressor, dialogue noise reduction, and limiter. Audio render runs the ordered stateful chain. Sample-range/stable-ID track peak/RMS follows the audio master; master peak/RMS and worker-owned EBU-R128 `LUFS-I` expose valid/analyzing/stale states. Background EBU-R128 analysis creates a reviewable revision/target-generation-bound change set for an editable −24 through −9 LUFS target, rejects unsafe gain, and applies one atomic batch. Device selection, per-device latency calibration, and polling/recovery are wired. Accelerated one-hour zero-xrun and two-hour less-than-one-frame drift simulations pass; physical-device matrix evidence and the stricter 10 ms gate remain release requirements. |
...
| Caption render/burn-in | Partial | Canonical alignment, normalized vertical position, safe margin, emphasis, colors, and outline are shared by the caption panel, schema-v3 project state, and deterministic burn-in before YUV conversion. `sans-serif` and mapped Noto family requests use pinned HarfBuzz/FreeType shaping with the bundled Noto Sans TTF; unknown families fall back to the deterministic 5×7 bitmap glyphs. Sidecars use atomic SRT/WebVTT serialization. Text subtitle streams (SubRip, WebVTT, mov_text) can be extracted from imported media into sequence captions; bitmap/styled streams (PGS, ASS, and similar) are reported as unsupported. Muxing captions back into exported media remains missing. |
...
| Background job protocol | Partial | Protobuf framing, IDs, cancellation registry, probe/thumbnail/waveform/proxy/export/transcribe jobs, and typed transcription/export options work. The desktop launches a restartable one-job worker host over framed standard I/O for proxy, export, transcription, and cache generation; cancellation sends in-flight `CancelJob` over duplex stdin and falls back to kill on write failure or timeout, and worker death still terminates without committing a partial destination. Named-pipe/Unix-socket routing is not implemented. |
...
The remaining Linux-first product blockers are physical Linux realtime audio/xrun/drift endurance on
the supported matrix; approved H.264/AAC; full color/LUT breadth; the Linux Vulkan hardware-lab
matrix; physical multilingual/Vulkan transcription validation; Flatpak identity, checksummed sources,
and store lint; the timed human accessibility/beginner study; security; and legal gates. Per-device
latency calibration now exists in QSettings, but the one-hour zero-xrun, two-hour drift, and 10 ms
lab protocol still require real hardware. Restartable proxy/export/transcription workers, automated
save/disk-full/GPU-loss/worker-death injection, and a synthetic 200+ corpus generator are in the tree.
