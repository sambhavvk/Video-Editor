...
- Deliver panel **Save recipe…** stores preset, destination, override indices, and hardware preference
  in `delivery/recipes` QSettings JSON.
- **Queue recipe** applies the saved settings and enqueues export jobs for the primary destination
  plus any extra destinations recorded in the recipe.
